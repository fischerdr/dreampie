...
class ConfigDialog(SimpleGladeApp):
    def __init__(self, config, gladefile, parent):
        ...
        self.themes = dict(
            (name, tags.get_theme(config, name))
        ...
        # tag, desc, fg, bg
        self.elements_list = gtk.ListStore(
    ...
    def run(self):
        ...
        cur_theme_name = [
            name for name, theme in self.themes.iteritems() if theme is self.cur_theme
        ][0]
    ...
    def init_elements_list(self):
        etv = self.elements_trv
        el = self.elements_list
        for i, (tag, desc) in enumerate(tags.tag_desc):
            el.insert(i, (tag, desc, None, None))
        letter = gtk.CellRendererText()
        letter.props.text = "A"
        etv.insert_column_with_attributes(
            0, "Preview", letter, foreground=2, background=3
        )
        name = gtk.CellRendererText()
        etv.insert_column_with_attributes(1, "Element Name", name, text=1)
    ...
    def init_textview(self):
        ...
        w(">>>", PROMPT)
        w(" ")
        w("# You can click here", COMMENT)
        w("\n")
        w("...", PROMPT)
        w(" ")
        w("# to choose elements!", COMMENT)
        w("\n")
        w("...", PROMPT)
        w(" ")
        w("def", KEYWORD)
    ...
    def on_viewer_button_clicked(self, _widget):
        ...
        open_dialog(
            f, _("Choose the viewer program"), self.config_dialog, _("Executables"), "*"
        )

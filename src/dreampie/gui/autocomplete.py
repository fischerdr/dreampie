...
class Autocomplete(object):
    def __init__(
        ...
        self.window = AutocompleteWindow(
            sourceview, sv_changed, window_main, self._on_complete
        )
    ...
    def _complete_filenames(self, text, index, hp, is_auto):
        ...
        if (
            ...
            and text[index - 1] == "\\"
            and not is_raw
            and not self._is_backslash_char(text, index - 1)
        ):
            return
        ...
        if sep_ind == -1 or sep_ind < str_start:
            ...
            text[sep_ind] == "\\"
            and not is_raw
            and not self._is_backslash_char(text, sep_ind)
        ):

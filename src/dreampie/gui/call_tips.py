...
class CallTips(object):
    def __init__(self, sourceview, sv_changed, window_main, get_func_doc, INDENT_WIDTH):
        ...
        self.start_mark = sb.create_mark(None, sb.get_start_iter(), left_gravity=True)
        self.end_mark = sb.create_mark(None, sb.get_start_iter(), left_gravity=False)
    ...
    def on_mark_set(self, sb, it, mark):
        if mark is sb.get_insert():
            if (
                it.compare(sb.get_iter_at_mark(self.start_mark)) < 0

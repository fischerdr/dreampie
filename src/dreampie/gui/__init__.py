...
from .keyhandler import handle_keypress, make_keyhandler_decorator, parse_keypress_event
from .newline_and_indent import newline_and_indent
from .output import Output
from .selection import Selection
from .SimpleGladeApp import SimpleGladeApp
from .status_bar import StatusBar
from .subprocess_handler import StartError, SubprocessHandler
from .tags import (
    COMMAND,
    COMMAND_DEFS,
    COMMAND_SEP,
...
class DreamPie(SimpleGladeApp):
    def __init__(self, pyexec, runfile):
        ...
        if self.config.get_bool("start-rpdb2-embedded"):
            ...
            print("Done.")
        self.window_main.set_icon_from_file(path.join(data_dir, "dreampie.png"))
        ...
        self.popup_mark = tb.create_mark(
            "popup-mark", tb.get_start_iter(), left_gravity=True
        )
        ...
        self.status_bar = StatusBar(self.sourcebuffer, self.sv_changed, self.statusbar)

        self.vadj_to_bottom = VAdjToBottom(
            self.scrolledwindow_textview.get_vadjustment()
        )

        self.history = History(
            self.textview, self.sourceview, self.sv_changed, self.config
        )

        self.recent_manager = gtk.recent_manager_get_default()
        self.menuitem_recent = [
            self.menuitem_recent0,
            self.menuitem_recent1,
        ...
        self.histpersist = HistPersist(
            self.window_main, self.textview, self.status_bar, self.recent_manager
        )
        self.update_recent()
        ...
        self.subp_welcome, self.subp_can_mask_sigint = self.call_subp(
            "get_subprocess_info"
        )
        self.show_welcome()
        self.configure_subp()
    ...
    def set_mac_accelerators(self):
        ...
        if sys.platform != "darwin":
            menu_up.add_accelerator("activate", accel_group, PGUP, gdk.CONTROL_MASK, 0)
            menu_dn.add_accelerator("activate", accel_group, PGDN, gdk.CONTROL_MASK, 0)
        else:
            menu_up.remove_accelerator(accel_group, UP, gdk.CONTROL_MASK)
            menu_up.add_accelerator(
                "activate", accel_group, PGUP, gdk.CONTROL_MASK, gtk.ACCEL_VISIBLE
            )
            menu_up.add_accelerator("activate", accel_group, UP, gdk.CONTROL_MASK, 0)

            menu_dn.remove_accelerator(accel_group, DN, gdk.CONTROL_MASK)
            menu_dn.add_accelerator(
                "activate", accel_group, PGDN, gdk.CONTROL_MASK, gtk.ACCEL_VISIBLE
            )
    ...
    def get_char_width_height(self):
        ...
        metrics = context.get_metrics(tv.style.font_desc, context.get_language())
    ...
    def on_textview_focus_in(self, _widget, _event):
        ...
        self.sourcebuffer.move_mark(
            self.sourcebuffer.get_selection_bound(),
            self.sourcebuffer.get_iter_at_mark(self.sourcebuffer.get_insert()),
        )

    def on_sourceview_focus_in(self, _widget, _event):
        # Clear the selection of the textbuffer
        self.textbuffer.move_mark(
            self.textbuffer.get_selection_bound(),
            self.textbuffer.get_iter_at_mark(self.textbuffer.get_insert()),
        )
    ...
    def set_is_executing(self, is_executing):
        ...
        label = _("Execute Code") if not is_executing else _("Write Input")
    ...
    def replace_gtk_quotes(source):
        ...
        # quotes.
        return source.replace("\xa8", '"').replace("\xb4", "'")
    ...
    def execute_source(self):
        ...
        try:
            ...
            self.call_subp_noblock("pause_idle")
        except TimeoutError:
            self.subp.send_object(("resume_idle", ()))
    ...
    def on_sourceview_return(self):
        ...
        if (
            insert_iter.equal(sb.get_end_iter())
            and insert_iter.get_line() == 0
            and insert_iter.get_offset() != 0
            and not get_text(sb, sb.get_start_iter(), insert_iter).endswith(" ")
        ):
            ...
            if not self.is_executing:
                ...
                source = self.replace_gtk_quotes(source)
                try:
                    is_incomplete = self.call_subp_noblock("is_incomplete", source)
                except TimeoutError:
                    is_incomplete = True
        ...
        if show_execution_tip:
            self.status_bar.set_status(_("Tip: To execute your code, use Ctrl+Enter."))
        return r
    ...
    def is_callable_only(self, expr):
        return self.call_subp_catch("is_callable_only", expr)
    ...
    def configure_subp(self):
        ...
        if config.get_bool("use-reshist"):
            ...
            reshist_size = 0
        self.call_subp("set_reshist_size", reshist_size)
        self.menuitem_clear_reshist.props.sensitive = reshist_size > 0

        self.call_subp("set_pprint", config.get_bool("pprint"))
    ...
    def run_init_code(self, runfile=None):
        ...
        if runfile:
            ...
            init_code += "\n\nprint(%r)\nexec(open(%r).read())\n" % (msg, runfile)
        if init_code:
            is_ok, syntax_error_info = self.call_subp("execute", init_code)
            if not is_ok:
                msg, lineno, offset = syntax_error_info
                warning = _(
        ...
        if not is_success:
            ...
            if val_str is not None:
                if val_no is not None:
                    sep = " " if "\n" not in val_str else "\n"
                    self.write_output(
                        "%d:%s" % (val_no, sep), RESULT_IND, onnewline=True
                    )
    ...
    def on_interrupt(self, _widget):
        if self.subp_can_mask_sigint or self.is_executing:
            ...
            self.status_bar.set_status(_("A command isn't being executed currently"))
            beep()
    ...
    def update_recent(self):
        ...
        recent_items = [
            it
            for it in rman.get_items()
            if it.has_application("dreampie") and it.get_uri().startswith("file://")
        ]
        ...
        recent_items.sort(key=lambda it: it.get_modified(), reverse=True)
    ...
    def on_section_menu_activate(self, widget):
        ...
        if widget is self.fold_unfold_section_menu:
            ...
            if widget is self.copy_section_menu:
                ...
                save_dialog(
                    func,
                    _("Choose where to save the section"),
                    self.main_widget,
                    _("All Files"),
                    "*",
                    None,
    ...
    def close_current_tab(self):
        ...
        text = get_text(
            self.sourcebuffer,
            self.sourcebuffer.get_start_iter(),
    ...
    def complete_dict_keys(self, expr):
        return self.call_subp_catch("complete_dict_keys", expr)

    def complete_attributes(self, expr):
        return self.call_subp_catch("complete_attributes", expr)

    def complete_firstlevels(self):
        return self.call_subp_catch("complete_firstlevels")

    def get_func_args(self, expr):
        return self.call_subp_catch("get_func_args", expr)

    def find_modules(self, expr):
        return self.call_subp_catch("find_modules", expr)

    def get_module_members(self, expr):
        return self.call_subp_catch("get_module_members", expr)

    def complete_filenames(self, str_prefix, text, str_char, add_quote):
        return self.call_subp_catch(
            "complete_filenames", str_prefix, text, str_char, add_quote
        )
    ...
    def get_func_doc(self, expr):
        return self.call_subp_catch("get_func_doc", expr)
    ...
    def on_clear_reshist(self, _widget):
        try:
            self.call_subp_noblock("clear_reshist")
        except TimeoutError:
    ...
    def quit(self):
        ...
        if self.textbuffer.get_modified() and (
            was_saved or self.config.get_bool("ask-on-quit")
        ):
            d = gtk.MessageDialog(
    ...
    def on_about(self, _widget):
        ...
        d.set_logo(gdk.pixbuf_new_from_file(path.join(data_dir, "dreampie.png")))
        d.run()
        d.destroy()
    ...
    def on_update_available(self, is_git, latest_name, latest_time):
        ...
        if is_git:
            msg = _(
                "A new git commit is available, from %s. "
                "Run 'git pull' to update." % date
            )
        else:
            self.get_update_menu.show()
            msg = _(
                "A new DreamPie version, %s, is available. "
    ...
    def show_popup_menu(self, event):
        ...
        if tb.get_has_selection():
            ...
            if r is not None:
                ...
                self.fold_unfold_section_menu.props.visible = is_folded is not None
                self.fold_unfold_section_menu.child.props.label = (
                    _("Unfold %s") if is_folded else _("Fold %s")
                ) % typ_s
                ...
                self.view_section_menu.props.visible = bool(
                    eval(self.config.get("viewer"))
                )

                tb.move_mark(self.popup_mark, it)
                self.popup_nosel_menu.popup(
...
def main():
    ...
    parser.add_option(
        "--run",
        dest="runfile",
        help="A file to run upon initialization. It will be " "run only once.",
    )
    if sys.platform == "win32":
        parser.add_option(
            "--hide-console-window",
    ...
    if len(args) == 1:
        ...
        msg = gtk.MessageDialog(
            ...
            gtk.BUTTONS_CLOSE,
            _(
                "DreamPie must be given the file name of a Python interpreter. "

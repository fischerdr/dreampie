...
class HistPersist(object):
    ...
    def recent_add(self, filename):
        ...
        self.recent_manager.add_full(
            "file://" + filename,
            {"mime_type": "text/html", "app_name": "dreampie", "app_exec": "dreampie"},
        )
...
<body>"""
    ...
    while True:
        ...
        shared_prefix = 0
        while (
            len(cur_tags) > shared_prefix
            and len(new_tags) > shared_prefix
...
class Parser(HTMLParser):
    ...
    def insert(self, data):
        tb = self.textbuffer
        leftmark = self.leftmark
        rightmark = self.rightmark
